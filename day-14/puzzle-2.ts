...
function simulateSandFalling(
  ...
  if (restPos === null || restPos === `${fallPoint[0]},${fallPoint[1]}`) {
    return false;
  }
...
ctx.maxY = ctx.maxY + 2;
const yRange = Math.abs(ctx.minY) + Math.abs(ctx.maxY);
ctx.minX = ctx.minX - yRange;
ctx.maxX = ctx.maxX + yRange;

const grid = buildGrid(ranges, ctx);
